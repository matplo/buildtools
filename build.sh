...
function setup_upack_dir()
{
	if [ $(bool ${BT_download}) ]; then
		...
		if [ -f "${BT_local_file}" ]; then
			...
			export BT_unpack_dir=${BT_working_dir}/${_local_dir}
			echo "[i] setup unpack_dir to ${BT_unpack_dir}"
		else
			if [ -z "${BT_unpack_dir}" ]; then
				echo "[w] local file does not exist? ${local_file}"
				export BT_unpack_dir=${BT_working_dir}/${BT_module}_${BT_version}
			fi
			env | grep BT_unpack_dir
		fi
		cd $savedir
	fi
}
...
function run_build()
{
	if [ $(bool ${BT_build}) ]; then
		...
		if [ ! -d "${BT_unpack_dir}" ]; then
			echo "[i] unpacking..."
			tar zxvf $local_file 2>&1 > /dev/null
		fi
		[ ! -d "${BT_unpack_dir}" ] && echo "[e] dir "${BT_unpack_dir}" does not exist" && exit 1
		cd "${BT_unpack_dir}"
